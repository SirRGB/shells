...
## (Enhanced) Thompson Shell
    ...
    # Move for usage
    mv /tmp/etsh-current-24/tsh "${INSTALL_PATH}"/tsh && \
    mv /tmp/etsh-current-24/etsh "${INSTALL_PATH}"/etsh" \
## yet another shell
...
# gsh
    "mv /tmp/gsh "${INSTALL_PATH}"/gsh" \
# reshell
    "mv /tmp/reshell "${INSTALL_PATH}"/reshell" \
# ion
